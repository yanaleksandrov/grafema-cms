...
class ResponseXml {
	...
	public function __construct(
		...
		$root = $this->createRootElement( $root );
	...
	public function addProcessingInstruction( string $target, string $data ): self {
		...
		$root = $elements->count() > 0 ? $elements->item( 0 ) : null;

		$processingInstruction = $this->document->createProcessingInstruction( $target, $data );

		$this->document->insertBefore( $processingInstruction, $root );
	...
	protected function createRootElement( string|array $root ): DOMElement {
		if ( is_string( $root ) ) {
			$rootName = $root ?: 'root';

			return $this->document->createElement( $rootName );
		}

		$rootName = $root['rootElementName'] ?? 'root';

		$element = $this->document->createElement( $rootName );
		...
		foreach ( $root as $key => $value ) {
			...
			$this->addAttributes( $element, $root[ $key ] );
		}
