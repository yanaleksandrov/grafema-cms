 ...
 *
 * The class contains the following groups of methods:
 *
 * - Open and create a file.
 * - Download a file or output it to the browser.
 * - Getting information about an image (aspect ratio, MIME type, size, orientation).
 * - Control the image size: resize, crop, rotate, and more).
 * - Applying effects to an image: transparency, contrast, blur, sharpen, and more.
 * - Overlay text and layers.
 *
 * Based on https://github.com/claviska/SimpleImage/tree/3.7.2 library
 *
 * Author:    Cory LaViska <https://github.com/claviska>.
...
class Image
{
    ...
    public const ERR_FONT_FILE = 2;

    public const ERR_FREETYPE_NOT_ENABLED = 3;

    public const ERR_GD_NOT_ENABLED = 4;

    public const ERR_INVALID_COLOR = 5;

    public const ERR_INVALID_DATA_URI = 6;

    public const ERR_INVALID_IMAGE = 7;

    public const ERR_LIB_NOT_LOADED = 8;

    public const ERR_UNSUPPORTED_FORMAT = 9;

    public const ERR_WEBP_NOT_ENABLED = 10;

    public const ERR_WRITE = 11;

    public const ERR_INVALID_FLAG = 12;
    ...
    public function setFlag($flag, $value)
    {
        ...
        if ( ! in_array($flag, array_keys($this->flags), true)) {
    ...
    {
        return in_array($flag, array_keys($this->flags), true) ? $this->flags[$flag] : null;
    }
    ...
    public function toFile($file, $mimeType = null, $quality = 100)
    {
        ...
        if (in_array($mimeType, array_keys($allowed_extension), true)) {
            $extension = image_type_to_extension($allowed_extension[$mimeType]);
     ...
     *
     * @return null|array returns an array of exif data or null if no data is available
     */
     ...
     * Crop the image.
     *
     * @param float|int $x1 top left x coordinate
     * @param float|int $y1 top left y coordinate
     ...
     *
     * @param Image|string $overlay                 The image to overlay. This can be a filename, a data URI, or a Image object.
     ...
     *
     * @param array|string $color the color to adjust
     ...
     * Darkens a color.
     *
     * @param array|string $color  the color to darken
     ...
     * Lightens a color.
     *
     * @param array|string $color  the color to lighten
     ...
     *
     * @param float|int $value a numeric value to test
     ...
     *
     * @param array|string $color the color to allocate
     *
     * @return int
     */
     ...
     * @param array|string $borderColor the color of border
     */
