        ...
        974: function(module) {
            (function(global, factory) {
                ...
                function validateTime(el, max) {
                    var _max = String(max), _pos = el.selectionStart;
                    var data = [ '^([0-9]' ];
                ...
                function deepExtend() {
                    ...
                    var obj = arg[0], other = arg.slice(1);
                ...
                var defaultOptions = {
                    ...
                    templates: {
                        month: [ '<div class="datepicker__month">', '<%= renderHeader() %>', '<%= renderMonth() %>', '</div>' ].join(''),
                        header: [ '<div class="datepicker__header">', '<a class="datepicker__prev<%= ( hasPrev ) ? "" : " is-disabled" %>" data-prev>&lsaquo;</a>', '<span class="datepicker__title"><%= renderMonthSelect() %></span>', '<span class="datepicker__title"><%= renderYearSelect() %></span>', '<a class="datepicker__next<%= ( hasNext ) ? "" : " is-disabled" %>" data-next>&rsaquo;</a>', '</div>' ].join(''),
                        time: [ '<div class="datepicker__timepicker">', '<%= renderTimepicker() %>', '</div>' ].join(''),
                        timepicker: [ '<div class="datepicker__timebox">', '<div class="datepicker__time">', '<%= renderHourSelect() %> : ', '<%= renderMinuteSelect() %>', '</div>', '<%= renderPeriodSelect() %>', '</div>' ].join(''),
                        calendar: [ '<div class="datepicker__box">', '<table>', '<thead>', '<tr>', '<% weekdays.forEach( function( name ) { %>', '<th><%= name %></th>', '<% } ); %>', '</tr>', '</thead>', '<tbody>', '<% days.forEach( function( day, i ) { %>', '<%= ( i % 7 == 0 ) ? "<tr>" : "" %>', '<%= renderDay( day ) %>', '<%= ( i % 7 == 6 ) ? "</tr>" : "" %>', '<% } ); %>', '</tbody>', '</table>', '</div>' ].join(''),
                        day: [ '<% classNames.unshift( "datepicker__day" ); %>', '<td class="<%= classNames.join( " " ) %>" data-day="<%= timestamp %>">', '<span><%= daynum %></span>', '</td>' ].join('')
                    }
                };
                ...
                function updateClassNames(classNames, opts) {
                    var nodeClass = classNames.node, inlineClass = classNames.inline, wrapperClass = classNames.wrapper;
                ...
                function translate(type, opts, index) {
                    ...
                    if (type === 'months') {
                        for (var month = 0; month < 12; month++) {
                            var date = new Date(Date.UTC(2e3, month, 1, 0, 0, 0)), name = new Intl.DateTimeFormat(opts.lang, {
                                month: 'long'
                            }).format(date);
                    ...
                    if (type === 'ampm') {
                        var startDate = new Date(Date.UTC(2e3, 1, 1, 0, 0, 0)), endDate = new Date(Date.UTC(2e3, 1, 1, 12, 0, 0));
                    ...
                    if (type === 'weekdays') {
                        for (var weekday = 1; weekday <= 7; weekday++) {
                            var date = new Date(Date.UTC(2020, 10, weekday, 0, 0, 0)), name = new Intl.DateTimeFormat(opts.lang, {
                                weekday: 'short'
                            }).format(date);
                            data.push(name[0].toUpperCase() + name.substring(1));
                        }
                ...
                var Datepicker = function() {
                    ...
                    createClass(Datepicker, [ {
                        ...
                        value: function _onmousedown(e) {
                            var opts = this._opts, ranged = opts.ranged, selector = opts.classNames.selected, dateNode = closest(e.target, '[data-day]', this.wrapper), date = dateNode ? parseInt(dateNode.dataset.day, 10) : null;
                        ...
                        value: function _onmousemove(e) {
                            var _this5 = this, opts = this._opts, ranged = opts.ranged, selector = opts.classNames.selected;
                        ...
                        value: function getValue() {
                            var opts = this._opts, toValue = opts.toValue;
                            var selected = [].concat(this.getDate() || []);
                        ...
                        value: function setValue(val) {
                            var opts = this._opts, fromValue = opts.fromValue;
                            this._selected = [];
                        ...
                        value: function dateAllowed(date, dim) {
                            var opts = this._opts, min = opts.min, max = opts.max, within = opts.within, without = opts.without;
                            var belowMax = void 0, aboveMin = belowMax = true;
                        ...
                        value: function render() {
                            var _this9 = this;
                            var opts = this._opts, time = opts.time, i = 0;
                        ...
                        value: function getData() {
                            ...
                            var opts = this._opts, i18n = opts.i18n;
                            var date = new Date(this._month.getTime());
                            ...
                            for (var i = 0; i < dayCount; i++) {
                                var day = new Date(year, month, 1 + (i - start)), dayMonth = day.getMonth(), weekday = day.getDay(), isSelected = this.hasDate(day), isDisabled = !this.dateAllowed(day), isPrevMonth = dayMonth < month, isNextMonth = dayMonth > month, isThisMonth = !isPrevMonth && !isNextMonth, isWeekend = weekday === 0 || weekday === 6, isToday = day.getTime() === today.getTime(), classNames = [];
                        ...
                        value: function _renderHeader(data) {
                            var _this10 = this;
                            var opts = this._opts, yearRange = opts.yearRange, i18n = opts.i18n;
                            var _date = data._date, index = data.index, year = data.year;
                        ...
                        value: function _renderTimepicker(name) {
                            var _this11 = this, opts = this._opts, i18n = opts.i18n;
                            if (!opts.time) return;
                            if (!this._time) {
                                this.setTime(true);
                            }
                            ...
                            return this._renderers.timepicker({
                                renderHourSelect: function renderHourSelect() {
                                    ...
                                    var options = [], hour = time[0], end = long ? 24 : 12;
                                ...
                                renderMinuteSelect: function renderMinuteSelect() {
                                    var step = _this11._opts.minutesStep, incr = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : step > 0 && step < 31 ? step : 15, options = [], minute = time[1];
